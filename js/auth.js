/* ============================================================
   E2E ENCRYPTED VAULT MODULE
   Client-side zero-knowledge task encryption using Web Crypto API
   (PBKDF2 + AES-256-GCM). Preserves local privacy without any server.
   ============================================================ */
import { $ } from './dom.js';
...
export function updateAuthBadge() {
  ...
  if (!badgeBtn) return;
  if (isVaultUnlocked()) {
    badgeBtn.innerHTML = `<i data-lucide="shield-check"></i> <span>Vault Unlocked (${currentUser || 'User'})</span>`;
    ...
    badgeBtn.innerHTML = `<i data-lucide="key"></i> <span>E2E Vault</span>`;
